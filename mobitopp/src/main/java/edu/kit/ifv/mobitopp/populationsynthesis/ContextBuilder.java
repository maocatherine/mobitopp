...
public class ContextBuilder {
    ...
    public ContextBuilder(AreaTypeRepository areaTypeRepository) {
        this(areaTypeRepository, DefaultMappings.noAutonomousModes(), StandardNetfileLanguages::english);
    }

    public ContextBuilder() {
        this(new BicRepository(), DefaultMappings.noAutonomousModes(),
                StandardNetfileLanguages::english);
    }
    ...
    private void dataRepository() throws IOException {
        ...
        DemandRegionMapping demandRegionMapping = loadDemandRegionMapping(); //Demand region is null here.
        //Here reads other data from the csv files.
    ...
    private void opportunity() {
        opportunityFile = Convert.asFile(configuration.getPois());
        log("Load opportunity file");
    }
    ...
    private SynthesisContext createContext() {
        ...
        SimpleSynthesisContext context = new SimpleSynthesisContext(configuration,
                experimentalParameters, network, roadNetwork, dataRepository, carSharing, carEngineFile, opportunityFile,
